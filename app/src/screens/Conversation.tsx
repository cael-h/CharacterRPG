...
export default function Conversation() {
  ...
  const [reviewProv, setReviewProv] = useState<'mock' | 'openai' | 'ollama'>(provider === 'ollama' ? 'ollama' : 'openai');
    ...
    try {
      const body = {
        ...
        characters: characters
          ...
          .map(c => ({ name: c.name, system_prompt: c.system_prompt || '' })),
        provider,
        model: provider === 'ollama' ? modelToOllamaId(model, customOllamaModel) : model,
        mature,
        tweakMode,
        useRag,
        reviewer_provider: reviewProv,
        reviewer_model: reviewModel,
        style_short: short
      };

      const response: ApiTurnResponse = await fetch(`${apiBase}/api/convo/turn`, {
        method: 'POST',
  ...
  return (
    <View style={{ flex: 1, padding: 12 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
        ...
        <View style={{ width: 8 }} />
        <Button
          title={`Reviewer: ${reviewProv}`}
          onPress={() =>
            setReviewProv(reviewProv === 'openai' ? 'ollama' : reviewProv === 'ollama' ? 'mock' : 'openai')
          }
        />
        <View style={{ width: 8 }} />
        <TextInput
          ...
          style={{ flex: 1, borderWidth: 1, borderColor: '#ccc', borderRadius: 6, padding: 6 }}
        />
        <View style={{ width: 8 }} />
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <TextInput
          ...
          placeholder="Type a message or /LLM ..."
