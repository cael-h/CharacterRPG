...
export default function Conversation() {
  const { apiBase, provider, model, sessionId, characters, selected, turns, set, pushTurn, startSession, incUsage } = useStore();
  ...
  const send = async () => {
    ...
    try {
      const r = await fetch(`${apiBase}/api/convo/turn`, {
        ...
        body: JSON.stringify({ session_id: sessionId, player_text: text, scene_context: {}, characters: characters.filter(c=>selected.includes(c.id)).map(c=>({name:c.name, system_prompt:c.system_prompt||''})), provider })
      }).then(r=>r.json());
