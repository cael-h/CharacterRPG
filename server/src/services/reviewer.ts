...
export async function reviewerSelect(opts: {
  ...
  const { reviewer_provider, reviewer_model, x_provider_key, candidates, style_short } = opts;
  ...
  const sys = 'You are a retrieval reviewer. Read the candidate snippets and return JSON {"selected":[ids],"notes":"...","ask_clarify":true|false}. Keep selected small (<=3).';
